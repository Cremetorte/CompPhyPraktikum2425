...
def solve_shock_tube(rho, u, epsilon, N, dt, dx, T_end, gamma):
    t = 0
    while t < T_end:
        rho_mean = np.zeros(N+5)
        rho_mean_new = np.zeros(N+5)

        rho_new, u_new, epsilon_new = np.copy(rho), np.copy(u), np.copy(epsilon)
        ...
        rho, u, epsilon = rho_new, u_new, epsilon_new
        t += dt

    return rho, u, epsilon