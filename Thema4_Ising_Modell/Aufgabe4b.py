...
def compute_observables(spins, J=1, h=0):
    ...
    return E / (L * L), M / (L * L), M_sq/L**2




@njit
def draw_heat_bath_variable_h(beta, L, N_try, N_thermalizing=1000, N_a=100, J=1, h=0):
    spins = initialize_lattice(L)
    h_space_half = np.linspace(-1, 1, 100)
    h_space = np.concatenate((h_space_half, h_space_half[::-1]))
    magnetizations = []

    # Thermalising
    for _ in range(N_thermalizing):
        spins = heat_bath_sweep(spins, beta, J=1, h=-1)
        
    for i in range(len(h_space)):
        # destroy auto correlartions
        for _ in range(N_a):
            spins = heat_bath_sweep(spins, beta, J=1, h=h_space[i])
        magnetizations.append(compute_observables(spins)[1])

    return h_space, magnetizations

# @njit
# def get_obs_arrays(nr_obs, beta, L, N_try, N_thermalizing=1000, N_a=1000, J=1, h=0):
#     energy, magnetization, mag_sq = [], [], []
#     while not (spins := next(draw_heat_bath_variable_h(beta, L, N_try, N_thermalizing, N_a, J, h))):
...
# def aufgabe_4b(beta, L, N_try=5, N_thermalizing=1000, N_a=100, J=1):
#     # hysteresekuve
...
h_space, magnetization = draw_heat_bath_variable_h(beta=0.7, L=128, N_try=5, N_thermalizing=1000, N_a=100, J=1, h=0)
# Plot der Ergebnisse
...
plt.title(f"Hysterese-Effekt für $L={128}$, $\\beta={0.7}$")
plt.grid()
